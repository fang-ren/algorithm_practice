...
characters and replace it with the third character. For example, if 'a' and 'c' are adjacent, they can replaced with 'b'. 
...
def stringReduction(S):
    if len(S) <= 1:
        return S
    first_letter = S[0]
    rest = stringReduction(S[1:])
    if first_letter != rest[0]:
        if len(rest) %2 == 0:
            return first_letter
        else:
            return 'abc'.replace(first_letter, '').replace(rest[0], '')
    if first_letter == rest[0]:
        return first_letter + rest

print stringReduction('abccaccba')
